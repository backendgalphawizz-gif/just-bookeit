...
| POST | `/v2/auth/register` | Same fields as web vendor register. `multipart`: `registration_token`, `owner_name`, `email`, `shop_name` (or `brand_name`), `service_types[]`, bank fields, `aadhar_front`/`aadhar_back` (required), optional `aadhar_number`, location, `gst_no`, `cover_image`, `profile_image`, `shop_logo`, `pan_card`. Returns Bearer `token`. Status `active`. |
