...
@section('content')
    ...
    <form method="GET" class="jb-filters">
        <div class="jb-filters-grid">
            ...
            <div class="jb-filters-field"><label class="jb-label">Status</label><select name="status" class="jb-select"><option value="">All</option>@foreach (\App\Models\Order::STATUSES as $s)<option value="{{ $s }}" @selected(request('status') === $s)>{{ \App\Models\Order::statusLabelFor($s) }}</option>@endforeach</select></div>
