...
<p style="border-bottom:1px solid var(--vp-border);padding-bottom:10px; margin:-.75rem 0 0.95rem;color:var(--vp-muted);font-size:.9rem;">Update your password to keep your account secure.</p>
...
<form method="POST" action="{{ route('vendor.settings.password') }}">
    ...
    <div class="vp-field">
        <label class="vp-label" for="password_confirmation">Confirm New Password <span class="vp-required">*</span></label>
        <div class="vp-input-icon-wrap">
            @include('vendor.partials.nav-icon', ['icon' => 'lock'])
            <input id="password_confirmation" type="password" name="password_confirmation" class="vp-input @error('password_confirmation') vp-input--error @enderror" placeholder="Confirm password" required minlength="8" autocomplete="new-password">
            <button type="button" class="vp-input-toggle" data-toggle-password aria-label="Show password">
                @include('vendor.partials.nav-icon', ['icon' => 'eye'])
            </button>
        </div>
