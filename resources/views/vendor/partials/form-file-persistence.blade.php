<script defer>
(function () {
    const DB_NAME = 'jb-vendor-form-files';
    const STORE = 'pending';
    const hasValidationErrors = @json(($errors ?? null)?->any() ?? false);
    let restored = false;

    function storageKey() {
        return 'vendor-register:' + window.location.pathname;
    }
    ...
    function findFileInput(name) {
        return Array.from(document.querySelectorAll('#vp-register-form input[type="file"]')).find((input) => input.name === name) ?? null;
    }
    ...
    function bindForm(form) {
        if (!form || form.dataset.vpFilePersistenceBound === '1') {
            return;
        }

        form.dataset.vpFilePersistenceBound = '1';
        ...
        form.addEventListener('submit', function (event) {
            ...
            if (form.dataset.vpFilePersisting === '1') {
                return;
            }

            event.preventDefault();
            form.dataset.vpFilePersisting = '1';

            captureFormFiles(form)
                .catch(() => {})
                .finally(() => {
                    delete form.dataset.vpFilePersisting;
