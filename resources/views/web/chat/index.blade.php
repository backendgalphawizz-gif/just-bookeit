            ...
            @if ($activeChat && $activeChat->vendor)
                ...
                <div class="jbw-chat-messages" id="jbw-chat-messages">
                    @forelse ($messages as $message)
                        <div @class(['jbw-chat-bubble', 'jbw-chat-bubble--mine' => $message->isFromCustomer(), 'jbw-chat-bubble--theirs' => ! $message->isFromCustomer()])>
                            @if ($message->body)
                                <p>{{ $message->body }}</p>
                            @endif
                            ...
                            @endif
                            <span class="jbw-chat-time">{{ $message->created_at?->format('g:i A') }}</span>
                        </div>
                    @empty
                ...
                <form method="POST" action="{{ route('web.chat.messages', $activeChat) }}" enctype="multipart/form-data" class="jbw-chat-compose">
                    @csrf
                    ...
                    <textarea name="body" rows="1" class="jbw-chat-input" placeholder="Type a message…">{{ old('body') }}</textarea>
                    <button type="submit" class="jbw-btn jbw-btn--primary jbw-btn--sm">Send</button>
                </form>
            @else
